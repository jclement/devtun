...
// The bottom border's right-hand chip carries the scroll position and, on the
// Config tab, which file an edit will land in. keyBar sized its own budget to
// six cells of chrome while borderIn reserves eight — four for the corners and
// their edge segments, then two more around each label — so at some widths the
// bar fitted its own budget, overran the border's, and the whole chip was
// silently dropped.
func TestTheBottomChipSurvivesEveryWidth(t *testing.T) {
	m := newTestModel(t, deps{tunnels: newStub(
		row(3000, 3000, "one"), row(3001, 3001, "two"), row(3002, 3002, "three"))})

	for width := minWidth; width <= 140; width++ {
		m.Update(tea.WindowSizeMsg{Width: width, Height: 24})
		chip := ansi.Strip(m.viewChip())
		if strings.TrimSpace(chip) == "" {
			continue
		}
		if !strings.Contains(ansi.Strip(m.frame()), chip) {
			t.Errorf("at %d columns the chip %q was dropped from the border", width, chip)
		}
	}
}
