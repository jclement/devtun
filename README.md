...
The page shows the same board the interface draws: the port table with its live counters, the security log streaming in, and — in two separate panels, because they are two different kinds of thing — the **grants** that are live now and lapsing on their own, and the **rules** that are written down and stay until revoked. You can hide a port, show the hidden ones again, name a port, pin its local port, pause and resume automatic forwarding, revoke a rule, rewrite one as a deny, revoke a grant, forget every grant at once, and ask for a reconnect — the same set the table and the Access tab offer, because under `--web` this is the surface and an action only the terminal has is one those users do not have. A rule that came from your config file says *edit your config file* instead of offering a button that would refuse, and **deny still only tightens**: an allow can be rewritten as a refusal from the page, never the other way round.

**It has the same five tabs the interface does** — Tunnels, Activity, Access, Services, Config — in the same order and under the same names, because two views of one session should not have to be learned twice. Click them, press `1`–`5`, or step with `[` and `]`; the one you were on is still there when you come back to the page. Each carries a count, and a waiting approval turns red on its tab, so putting a panel behind a tab does not mean losing sight of what is on it.

It also has the two screens the interface has and the board used to lack. **Services** says what devtun is layering onto the connection, whether each is switched on for this host, whether it is actually running, and — the part that matters — *why not* when it isn't: `no gpg-agent on the remote box` is a thing you can go and fix, and `off` on its own is not. **Settings** is every setting devtun has, each one saying which file decided its value: this host, your global file, or devtun's own default. The button in the corner says which of those two files your next change lands in, because setting something for the box in front of you and setting it for every box you ever connect to are different decisions.

There is exactly one description of what those settings are — `internal/settings`, which the terminal reads too. Two lists would drift, and the way you would find out is changing something on one screen and watching the other disagree about what it now is.

It runs *alongside* whichever interface owns your terminal rather than instead of it, so `--web` and the TUI are a fine combination — one on the second monitor, one in the pane.
